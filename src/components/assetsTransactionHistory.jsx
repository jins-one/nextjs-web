...
export const AssetsTransactionHistory = ({ onModal, closeModal, ...props }) => {
    ...
    async function history() {
        let api_url =  '/api/fetch/transaction-history';
    ...
    return (
        <>
            <WithdrawalWait onModal={withdrawlWaitModal} closeModal={setWithdrawlWaitModal} price={estimatePrice} fee={withdrawalFee} />
            <HistoryDetailAlert onModal={detailModal} closeModal={setDetailModal} text={detail} />
            ...
            <dialog ref={modalRef} className={styles.modal} onCancel={test}>
                ...
                <div className={styles.dialogContainer}>
                    <div className={styles.titleBox}>
                        <h1 className={styles.title}>입출금 내역</h1>
                    </div>
                    ...
                    <div className={styles.contentsBox}>
                        ...
                        <div className={`${styles.product} ${styles.searchOptionBar} ${styles.jusfiBet}`}>
                            ...
                            <div className={styles.searchOptionBar}>
                                ...
                                <select className={styles.dateSelect} value={type} onChange={(e) => setType(e.target.value)} required>
                                    <option value='default' disabled>종류</option>
                            ...
                            {/* checkbox start */}
                            <div className={styles.searchOptionBar}>
                                {/* <div className={styles.checkBoxContainer} onClick={handleAllChecked}>
                                    <input type="checkbox" checked={isAllChecked} onChange={() => { }} />
                                    <span className={styles.ml12}>전체</span>
                                </div>

                                <div className={`${styles.checkBoxContainer} ${styles.ml24}`} onClick={handleDepositChecked}>
                                    <input type="checkbox" checked={isDepositChecked} onChange={() => { }} />
                                    <span className={styles.ml12}>입금내용만</span>
                                </div>

                                <div className={`${styles.checkBoxContainer} ${styles.ml24}`} onClick={handleWithdrawalChecked}>
                                    <input type="checkbox" checked={isWithdrawalChecked} onChange={() => { }} />
                                    <span className={styles.ml12}>출금내용만</span>
                                </div> */}
                        ...
                        <div className={`${styles.oneRowContainer} ${styles.backGray} ${styles.mt12}`}>
                            <p className={`${styles.width33}`}>구분</p>
                            <p className={`${styles.width89}`}>거래상태</p>
                            <p className={`${styles.width200}`}>신청일</p>
                            <p className={`${styles.width130}`}>금액(ETH)</p>
                            <p className={`${styles.width130}`}>수수료(ETH)</p>
                            <p className={`${styles.width200}`}>처리일</p>
                            <p className={`${styles.width172}`}>상세내역</p>
                            <p className={`${styles.width107}`}>출금관리</p>
                        </div>
                        {/* pagination start */}
                        <div>
                            {
                                (historyList && historyList.length !== 0) ? historyList.map((h, index) => (
                                    ...
                                    <button className={styles.rowBtn} key={index}>
                                        <div className={`${styles.oneRowContainer}`}>
                                            <p className={`${styles.width33}`}>{h.type}</p>
                                            <p className={`${styles.width89}`} type={h.status}>{h.status}</p>
                                            <p className={`${styles.width200}`}>{h.s_date}</p>
                                            <p className={`${styles.width130}`}>{h.price}</p>
                                            <p className={`${styles.width130}`}>{h.fee}</p>
                                            <p className={`${styles.width200}`}>{h.p_date}</p>
                                            <p className={`${styles.width172}`} style={{ cursor: 'pointer' }}
                                                onClick={() => { setDetail(h.detail); setDetailModal(true); }}
                                            >{h.detail}</p>
                                            {
                                                h.status == '출금대기' ?
                                                    <p className={`${styles.width107} ${styles.withdrawalManageBtn}`}
                                                        onClick={() => { setEstimatePrice(h.price); setWithdrawalFee(h.fee); setWithdrawlWaitModal(true); }}>출금관리</p>
                                                    :
                                                    <></>
                                            }
                        ...
                        <div className={`${styles.infoBox} ${styles.mt80}`}>
                            <div className={styles.infoBoxTitleBox}>
                                ...
                                <span className={styles.ml12}>입출금관련 안내사항</span>
                            </div>
